...
{
    return mngr.setLedScheduler(data);
}
...
void setup() 
{
    ...
    Particle.function("setLedScheduler", setLedScheduler);
...
void loop() 
{
    ...
    // Update current time variable every second
    static unsigned long lastTimeUpdate = 0;
    if (millis() - lastTimeUpdate >= 1000)
    {
        currentTime = Time.format(Time.local(), "%Y-%m-%d %H:%M:%S");
